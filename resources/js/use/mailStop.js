...
const stopMailLoading = ref(false)

// const smsConfirmSend = async(code) => {
const StopMail = async(email) => {

    stopMailLoading.value = true

    let ress = await axios

        .post('/api/emailStop', { 'email': email })
        // 
        .then((response) => {
            console.log('mailStop', email, response.data)
            ress = response.data
                // console.log('mailStop2', response.data.data.result)
...
export default function mailStop() {
    return {
        stopMailLoading,
        StopMail
        // phone,
